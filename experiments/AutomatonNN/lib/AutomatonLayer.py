...
class AutomatonLayer(torch.nn.Module):
    def __init__(self, batch_size, input_size, output_size, num_states, device='cuda'):
        ...
        self.l_selection = torch.nn.Linear(self.input_size + self.output_size, self.num_states).to(device)
    ...
    def forward(self, x0):
        selection = self.softmax(self.l_selection(torch.cat((x0, self.last_state), 1)))
