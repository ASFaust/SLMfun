...
configs = json.load(open('rnn_configs_03.json', 'r'))

while True:
    print("\nstarting a new run")

    results_folder_contents = os.listdir("results_03")
    ...
    while True:
        ...
        if not (f"{current_config['unique id']}.json" in results_folder_contents):
            break

    #we chose a config that has not been run yet

    #we now save the config to the results folder
    with open(f"results_03/{current_config['unique id']}.json", "w") as f:
    ...
    del rnn_params["run 1 id"]
    del rnn_params["run 2 id"]
    del rnn_params["common id"]
    del rnn_params["unique id"]
    del rnn_params["training"]
    ...
    with open(f"results_03/{current_config['unique id']}.json", "r") as f:
