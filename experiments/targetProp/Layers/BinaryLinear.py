...
class BinaryLinear:
    def __init__(self, input_size, output_size, device):
        ...
        self.x = None
        self.binary_x = None
        self.activation = lambda x: torch.sign(x).clamp(min=0)
        self.alpha = 1.0
        self.dw = None
    ...
    def forward(self, x):
        with torch.no_grad():
            ...
            self.binary_x = self.activation(x).clone()  # Shape: (batch_size, input_size)
            self.v = torch.matmul(self.binary_x, self.weights)  # Shape: (batch_size, output_size)
        return self.v
    ...
    def backward(self, v_hat):
        with torch.no_grad():
            ...
            dw_pre = self.alpha * torch.matmul(self.binary_x.t(), dv)  # Shape: (input_size, output_size)

            # Compute preliminary updated weights
            w_hat_pre = self.weights + dw_pre  # Shape: (input_size, output_size)

            # Compute norms
            norm_x_sq = torch.sum(self.binary_x ** 2, dim=1, keepdim=True)  # Shape: (batch_size, 1)
            ...
            # apply activation to get realistic x_hat
            x_hat_mean = self.activation(x_hat_mean)
